...
const Bulty = ({ getWriteShopId }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await fetchBulties(getShopQueryParams());
        setBulties(data);
      } catch (err) {
        console.error('Failed to load bulty records', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!shopId) {
      alert('Please select a specific shop before saving bulty records.');
      return;
    }

    try {
      if (editId) {
        const updated = await updateBulty(editId, formData);
        setBulties(bulties.map((b) => (b.id === editId ? updated : b)));
      } else {
        const created = await createBulty(formData, shopId);
        setBulties([created, ...bulties]);
      }
      resetForm();
    } catch (err) {
      alert(err.response?.data?.message || 'Failed to save bulty record');
    }
  };
  ...
  const handleEdit = (record) => {
    setFormData({
      ...
      date: record.date,
      bultyNo: record.bultyNo,
      item: record.item,
      qty: record.qty,
      weight: String(record.weight),
      weightRate: String(record.weightRate),
      mazduri: String(record.mazduri),
      lifterCharges: String(record.lifterCharges),
      localRent: String(record.localRent),
      nakadKharcha: String(record.nakadKharcha),
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this bulty record?')) return;
    try {
      await deleteBulty(id);
  ...
  const filteredBulties = bulties.filter(
    (b) =>
                      ...
                      <div className="flex justify-between font-bold text-slate-800 dark:text-white text-base pt-2 border-t border-blue-200 dark:border-blue-800">
                        ...
                        <span>
                          ...
                          {calculateInTotal(
                            ...
                            formData.weightRate,
                            formData.mazduri,
                            formData.lifterCharges,
                            formData.localRent,
                            formData.nakadKharcha
                          ).toLocaleString()}
                        </span>
                      </div>
